...
/*
AIM : Write and Execute SQL indexing queries for data warehouse.
...
CREATE INDEX BTREE_CUST_GENDER ON CUSTOMERS_COPY_BTREE(CUST_GENDER);

CREATE INDEX BTREE_CUST_YOB ON CUSTOMERS_COPY_BTREE(CUST_YEAR_OF_BIRTH);

CREATE INDEX BTREE_CUST_LNAME ON CUSTOMERS_COPY_BTREE(CUST_LAST_NAME);

CREATE INDEX BTREE_CUST_STADDRESS ON CUSTOMERS_COPY_BTREE(CUST_STREET_ADDRESS);

/*
SQL> CREATE INDEX BTREE_CUST_GENDER ON CUSTOMERS_COPY_BTREE(CUST_GENDER);

Index created.

Elapsed: 00:00:00.03
SQL> CREATE INDEX BTREE_CUST_YOB ON CUSTOMERS_COPY_BTREE(CUST_YEAR_OF_BIRTH);

Index created.

Elapsed: 00:00:00.03
SQL>
SQL> CREATE INDEX BTREE_CUST_LNAME ON CUSTOMERS_COPY_BTREE(CUST_LAST_NAME);

Index created.

Elapsed: 00:00:00.03
SQL>
SQL> CREATE INDEX BTREE_CUST_STADDRESS ON CUSTOMERS_COPY_BTREE(CUST_STREET_ADDRESS);
...
CREATE BITMAP INDEX BITMAP_CUST_GENDER ON CUSTOMERS_COPY_BITMAP(CUST_GENDER);

CREATE BITMAP INDEX BITMAP_CUST_YOB ON CUSTOMERS_COPY_BITMAP(CUST_YEAR_OF_BIRTH);

CREATE BITMAP INDEX BITMAP_CUST_LNAME ON CUSTOMERS_COPY_BITMAP(CUST_LAST_NAME);

CREATE BITMAP INDEX BITMAP_CUST_STADDRESS ON CUSTOMERS_COPY_BITMAP(CUST_STREET_ADDRESS);

-- HIGH CARDINALITY LEADS TO MORE TIME IN BITMAP

DROP INDEX BITMAP_CUST_GENDER;

DROP INDEX BITMAP_CUST_YOB;

DROP INDEX BITMAP_CUST_LNAME;

DROP INDEX BITMAP_CUST_STADDRESS;

/*
SQL> CREATE BITMAP INDEX BITMAP_CUST_GENDER ON CUSTOMERS_COPY_BITMAP(CUST_GENDER);--low cardinality takes less time in BITMAP

Index created.

Elapsed: 00:00:00.01
SQL>
SQL> CREATE BITMAP INDEX BITMAP_CUST_YOB ON CUSTOMERS_COPY_BITMAP(CUST_YEAR_OF_BIRTH);

Index created.

Elapsed: 00:00:00.03
SQL>
SQL> CREATE BITMAP INDEX BITMAP_CUST_LNAME ON CUSTOMERS_COPY_BITMAP(CUST_LAST_NAME);

Index created.

Elapsed: 00:00:00.01
SQL>
SQL> CREATE BITMAP INDEX BITMAP_CUST_STADDRESS ON CUSTOMERS_COPY_BITMAP(CUST_STREET_ADDRESS);
...
-- HIGH CARDINALITY CHECK FOR BTREE AND BITMAP
...
FOR i IN 1..2000000
LOOP
	INSERT INTO TEST_TABLE VALUES(i, MOD(i,2));
END LOOP;
END;
/
...
CREATE INDEX TEST_BTREE ON TEST_TABLE(COL1);

CREATE BITMAP INDEX TEST_BITMAP ON TEST_TABLE2(COL1);

/*
SQL> CREATE INDEX TEST_BTREE ON TEST_TABLE(COL1);

Index created.

Elapsed: 00:00:01.26
SQL>
SQL> CREATE BITMAP INDEX TEST_BITMAP ON TEST_TABLE2(COL1);
...
*/
-- LOW CARDINALITY
CREATE INDEX TEST_BTREE_COL2 ON TEST_TABLE(COL2);

CREATE BITMAP INDEX TEST_BITMAP_COL2 ON TEST_TABLE2(COL2);


/*
SQL> CREATE INDEX TEST_BTREE_COL2 ON TEST_TABLE(COL2);

Index created.

Elapsed: 00:00:01.21
SQL>
SQL> CREATE BITMAP INDEX TEST_BITMAP_COL2 ON TEST_TABLE2(COL2);
...
SELECT SEGMENT_NAME, BYTES/(1024*1024) AS "SIZE IN MB" FROM USER_SEGMENTS WHERE SEGMENT_NAME IN ('CUSTOMERS_COPY_BTREE', 'CUSTOMERS_COPY_BITMAP');

/*
SEGMENT_NAME                                                                      SIZE IN MB
--------------------------------------------------------------------------------- ----------
CUSTOMERS_COPY_BITMAP                                                                     12
CUSTOMERS_COPY_BTREE                                                                      12
*/

SELECT SEGMENT_NAME, BYTES/(1024*1024) AS "SIZE IN MB" FROM USER_SEGMENTS WHERE SEGMENT_NAME LIKE '%_CUST_%';

/*
SEGMENT_NAME                                                                      SIZE IN MB
--------------------------------------------------------------------------------- ----------
BITMAP_CUST_GENDER                                                                     .0625
BITMAP_CUST_LNAME                                                                       .125
BITMAP_CUST_STADDRESS                                                                      3
BITMAP_CUST_YOB                                                                        .1875
BTREE_CUST_GENDER                                                                       .875
BTREE_CUST_LNAME                                                                           2
BTREE_CUST_STADDRESS                                                                       3
BTREE_CUST_YOB                                                                             1
...
SELECT SEGMENT_NAME, BYTES/(1024*1024) AS "SIZE IN MB" FROM USER_SEGMENTS WHERE SEGMENT_NAME IN ('TEST_TABLE', 'TEST_TABLE2');
...
SELECT SEGMENT_NAME, BYTES/(1024*1024) AS "SIZE IN MB" FROM USER_SEGMENTS WHERE SEGMENT_NAME LIKE '%TEST_%';

/*
SEGMENT_NAME                                                                      SIZE IN MB
--------------------------------------------------------------------------------- ----------
TEST_BITMAP                                                                               57
TEST_BITMAP_COL2                                                                         .75 -- BITMAP WORKS AMAZINGLY IN LOW CARDINALITY
TEST_BTREE                                                                                37
...
SELECT COUNT(DISTINCT(CUST_YEAR_OF_BIRTH)) AS 'COUNT_YOB' FROM CUSTOMERS_COPY_BTREE;
...
-- YEAR_OF_BIRTH IS LOW CARDINALITY AS 75 IN 55400 TOTAL COUNT

SELECT * FROM CUSTOMERS_COPY_BTREE WHERE CUST_YEAR_OF_BIRTH = '1975'; -- HERE IN BTREE COST IS 406
...
------------------------------------------------------------------------------------------
|   0 | SELECT STATEMENT  |                      |   954 |   277K|   406   (1)| 00:00:05 |
|*  1 |  TABLE ACCESS FULL| CUSTOMERS_COPY_BTREE |   954 |   277K|   406   (1)| 00:00:05 |
------------------------------------------------------------------------------------------

Predicate Information (identified by operation id):
---------------------------------------------------

   1 - filter("CUST_YEAR_OF_BIRTH"=1975)
...
----------------------------------------------------------
       ...
       1520  consistent gets
          0  physical reads
          0  redo size
     152063  bytes sent via SQL*Net to client
...
SELECT * FROM CUSTOMERS_COPY_BITMAP WHERE CUST_YEAR_OF_BIRTH = '1975';	--HERE IN BITMAP COST IS 130

/*
Execution Plan
----------------------------------------------------------
Plan hash value: 3680933571

------------------------------------------------------------------------------------------------------
| Id  | Operation                    | Name                  | Rows  | Bytes | Cost (%CPU)| Time     |
------------------------------------------------------------------------------------------------------
|   0 | SELECT STATEMENT             |                       |   954 |   277K|   130   (0)| 00:00:02 |
|   1 |  TABLE ACCESS BY INDEX ROWID | CUSTOMERS_COPY_BITMAP |   954 |   277K|   130   (0)| 00:00:02 |
|   2 |   BITMAP CONVERSION TO ROWIDS|                       |       |       |            |          |
|*  3 |    BITMAP INDEX SINGLE VALUE | BITMAP_CUST_YOB       |       |       |            |          |
------------------------------------------------------------------------------------------------------

Predicate Information (identified by operation id):
---------------------------------------------------

   3 - access("CUST_YEAR_OF_BIRTH"=1975)
...
----------------------------------------------------------
       ...
       1022  consistent gets
          7  physical reads
          0  redo size
     192849  bytes sent via SQL*Net to client
       1212  bytes received via SQL*Net from client
         65  SQL*Net roundtrips to/from client
          0  sorts (memory)
          0  sorts (disk)
...
CREATE INDEX BTREE_CUST_ID ON CUSTOMERS_COPY_BTREE(CUST_ID);
CREATE BITMAP INDEX BITMAP_CUST_ID ON CUSTOMERS_COPY_BITMAP(CUST_ID);

SELECT SEGMENT_NAME, BYTES/(1024*1024) AS "SIZE IN MB" FROM USER_SEGMENTS WHERE SEGMENT_NAME LIKE '%_CUST_ID%';
...
CUST_YOB_VALUE NUMBER(4);
BEGIN
FOR I IN 1..5000 LOOP
UPD_CUST_ID := DBMS_RANDOM.VALUE(1, 55400);
CUST_YOB_VALUE := DBMS_RANDOM.VALUE(1800, 2020);
UPDATE CUSTOMERS_COPY_BTREE
SET CUST_YEAR_OF_BIRTH = CUST_YOB_VALUE
...
SQL> DECLARE
  ...
  6  UPD_CUST_ID := DBMS_RANDOM.VALUE(1, 55400);
  7  CUST_YOB_VALUE := DBMS_RANDOM.VALUE(1800, 2020);
...
--6
EXEC DBMS_STATS.gather_table_stats('SARTHAK', 'STUDENT');
